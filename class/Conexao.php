...
class Conexao{
    ...
    public static $enderecos;
    public static function InstEndereco() {
        $srv = "10.40.10.6";
        $usr = 'root';
        $db = 'enderecos';
        $pwd = 'semad@cpd';
        $dsn = 'mysql:dbname='.$db.';host='.$srv;     
        if (!isset(self::$enderecos)) {
            self::$enderecos = new PDO($dsn, $usr, $pwd ,array(PDO::MYSQL_ATTR_INIT_COMMAND => "SET NAMES utf8"));
        }
